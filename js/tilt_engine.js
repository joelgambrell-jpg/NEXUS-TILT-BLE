window.NEXUSTiltEngine = class {
  ...
  restore(snapshot) {
    ...
    this.plan = structuredClone(snapshot.plan);
    this.currentIndex = Number(snapshot.currentIndex) || 0;
    this.accepted = structuredClone(snapshot.accepted || []);
    this.candidate = snapshot.candidate ? structuredClone(snapshot.candidate) : null;
    this.testEnd = snapshot.testEnd ? structuredClone(snapshot.testEnd) : null;
  ...
  accept({ technicianName = 'POC User' } = {}) {
    ...
    const record = {
      ...
      recordVersion: 1,
      recordId: `ARC-${Date.now()}-${String(this.currentIndex + 1).padStart(2, '0')}`,
      runId: this.runId,
  ...
  endTest({ reason, technicianName = 'POC User' } = {}) {
    if (!this.plan || !['WAITING', 'REVIEW'].includes(this.status)) return null;
    const cleanReason = String(reason || '').trim();
    if (!cleanReason) throw new Error('End Test reason is required');
